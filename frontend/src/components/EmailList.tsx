...
//   return (
//     <div className="w-96 bg-background border-r border-border flex-shrink-0">
//       {/* Header */}
...
//                   from: email.from
//                 });
//                 console.log('📞 Calling onEmailSelect with ID:', email.id);
...
export function EmailList({
  ...
  if (isLoading) {
    return (
      <div className="w-[450px] flex-shrink-0 bg-background border-r border-border flex items-center justify-center">
  ...
  return (
    <div className="w-[450px] bg-background border-r border-border flex flex-col flex-shrink-0">
      <div className="p-6 border-b border-border">
        <h2 className="text-2xl font-bold text-foreground">
          {selectedFolder.charAt(0).toUpperCase() + selectedFolder.slice(1)}
        </h2>
        <p className="text-muted-foreground mt-1">{emails.length} emails</p>
      </div>
          ...
          {emails.map((email) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-foreground truncate">{email.from}</h3>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="text-xs text-muted-foreground">{email.timestamp}</span>
                      <Star className={cn("h-4 w-4", email.isStarred ? "text-yellow-500 fill-current" : "text-muted-foreground")} />
                    </div>
                  </div>
                  <h4 className="text-sm font-semibold mb-1 truncate">{email.subject}</h4>
